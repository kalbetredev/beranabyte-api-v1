...
export const getMyBio = async (req: Request, res: Response) => {
  Config.findOne({ key: MY_BIO_KEY }).then(async (config: any) => {
    if (config) {
      BioModel.findById(config.value).then(async (bio: any) => {
...
const saveAndReturnMyBio = async (req: Request, res: Response) => {
  ...
  bio.save().then(async (savedBio: any) => {
    if (!savedBio)
      return res.status(200).json({
        success: false,
    ...
    const config = new Config({ key: MY_BIO_KEY, value: savedBio._id });
    await config.save().then((config: any) => {
      if (!config)
        return res.status(200).json({
          success: false,
          msg: "Error Getting Bio",
